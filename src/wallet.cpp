...
// MODERCASH: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-MODERCASH-address
...
// MODERCASH: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CMODERCASHAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CMODERCASHAddress(address).ToString());
}
...
// MODERCASH: check 'spent' consistency between wallet and txindex
// MODERCASH: fix wallet spent state according to txindex
...
// MODERCASH: disable transaction (only for coinstake)
