...
{
    QString ret = QString("MODERCASH:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MDDC unit input in lnReqAmount, we generate the URI with MDDC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(MODERCASHUnits::format(MODERCASHUnits::MDDC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
